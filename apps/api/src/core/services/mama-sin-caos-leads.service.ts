...
import { ensureMamaSinCaosLeadsTable } from '../bootstrap/ensure-mama-sin-caos-leads-table';

export interface MamaSinCaosLeadRecord {
  id: string;
  email: string;
  firstName: string;
  phone: string;
...
const DEFAULT_SOURCE = 'landing-afluence-mama-sin-caos-lista-secreta';

function rowToRecord(row: MamaSinCaosLeadRow): MamaSinCaosLeadRecord {
  const createdAt =
...
async function withClient<T>(fn: (client: Client) => Promise<T>) {
  await ensureMamaSinCaosLeadsTable();
...
export async function upsertMamaSinCaosLead(input: {
  email: string;
  firstName?: string;
  phone?: string;
  source?: string;
  ...
  return withClient(async (client) => {
    const existing = await client.query<MamaSinCaosLeadRow>(
      `SELECT id, email, first_name, phone, source, custom_fields, utm_data, created_at, updated_at
       FROM marketing.mama_sin_caos_leads
       WHERE lower(email) = $1
       LIMIT 1`,
      [email],
    );
    ...
    if (existing.rows[0]) {
      ...
      const updated = await client.query<MamaSinCaosLeadRow>(
        `UPDATE marketing.mama_sin_caos_leads
         SET first_name = $2,
             phone = $3,
             source = $4,
    ...
    const created = await client.query<MamaSinCaosLeadRow>(
      `INSERT INTO marketing.mama_sin_caos_leads
...
export async function listMamaSinCaosLeadsForTable() {
  return withClient(async (client) => {
    const result = await client.query<MamaSinCaosLeadRow>(
      `SELECT id, email, first_name, phone, source, custom_fields, utm_data, created_at, updated_at
       FROM marketing.mama_sin_caos_leads
