...
import { ensureGermanRozProgressTable } from '../bootstrap/ensure-german-roz-progress-table';

export interface GermanRozProgressRecord {
  id: string;
  sessionId: string;
  source: string;
  email: string;
...
function rowToRecord(row: GermanRozProgressRow): GermanRozProgressRecord {
  return {
    id: row.id,
...
async function withClient<T>(fn: (client: Client) => Promise<T>) {
  await ensureGermanRozProgressTable();
...
export async function upsertGermanRozProgress(input: {
  sessionId: string;
  source?: string;
  email?: string;
  firstName?: string;
  ...
  leadId?: string;
  completedAt?: string | null;
}): Promise<GermanRozProgressRecord> {
  ...
  return withClient(async (client) => {
    const existing = await client.query<GermanRozProgressRow>(
      `SELECT *
       FROM marketing.german_roz_progress
       WHERE session_id = $1
       LIMIT 1`,
      [sessionId],
    );
    ...
    const source = (input.source ?? existing.rows[0]?.source ?? 'landing-german-roz-webinar-2026-06-10').trim();
    ...
    if (existing.rows[0]) {
      const result = await client.query<GermanRozProgressRow>(
        `UPDATE marketing.german_roz_progress
         SET email = $2,
             first_name = $3,
             phone = $4,
             source = $5,
    ...
    const result = await client.query<GermanRozProgressRow>(
      `INSERT INTO marketing.german_roz_progress
...
export async function listGermanRozProgressForTable() {
  return withClient(async (client) => {
    const result = await client.query<GermanRozProgressRow>(
      `SELECT *
       FROM marketing.german_roz_progress
