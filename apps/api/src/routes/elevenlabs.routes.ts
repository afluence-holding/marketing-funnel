    ...
    try {
      switch (event.type) {
        ...
        case 'post_call_audio':
          // Audio events can be handled later (e.g., store recording)
          break;
      }
...
const callSchema = z.object({
  ...
  orgId: z.string().uuid(),
  funnelEntryId: z.string().uuid().optional(),
...
router.post('/call', validate(callSchema), async (req, res) => {
  try {
    const { leadId, orgId, funnelEntryId, firstMessage, dynamicVariables, agentId } = req.body;
