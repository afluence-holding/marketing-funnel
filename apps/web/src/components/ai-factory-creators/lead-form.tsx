...
import { META_EVENTS, trackEvent } from '@/lib/tracking/events';
...
const facturaciones = ['Aún no monetizo', '0–1K USD', '1–5K USD', '5–20K USD', '20K+ USD'];
...
const DEBUG_BOOKING = process.env.NEXT_PUBLIC_DEBUG_BOOKING === '1';
...
export interface CalendarMeetingUpdatePayload {
  leadId: string;
  entryId?: string;
}
...
export type CalendarPrefill = {
  name: string;
  email: string;
  meetingUpdate?: CalendarMeetingUpdatePayload;
};

interface LeadFormProps {
  onCalendarStateChange: (state: CalendarState) => void;
  onCalendarUnlocked?: (prefill: CalendarPrefill) => void;
  calendarState: CalendarState;
}

export function LeadForm({ onCalendarStateChange, onCalendarUnlocked, calendarState }: LeadFormProps) {
  ...
  const handleBack = () => {
    if (step === 1) return;
    if (calendarState === 'rejected' && step === 5) onCalendarStateChange('locked');
    if (calendarState === 'rejected-whatsapp' && step === 5) onCalendarStateChange('locked');
    setDirection(-1);
    setStep(step - 1);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError('');
      const nameParts = data.nombre.trim().split(/\s+/).filter(Boolean);
      const firstName = nameParts[0] ?? '';
      const lastName = nameParts.slice(1).join(' ');
      const normalizedPhone = `${data.countryCode}${data.whatsapp}`.replace(/[^\d+]/g, '');
      if (!EMAIL_REGEX.test(data.email.trim())) throw new Error('Ingresa un correo valido');
      if (normalizedPhone.replace(/\D/g, '').length < 8) throw new Error('Ingresa un telefono valido');
      ...
      const body = {
        ...
        customFields: {
          link_social: data.linkSocial.trim(),
          nicho: data.nicho,
          genera_ingresos: data.generaIngresos,
          facturacion: data.facturacion,
          que_construir: JSON.stringify(data.queConstruir),
          inversion: data.inversion,
          timing: data.timing,
          confirma_whatsapp: data.confirmaWhatsapp,
          form_type: 'full',
        },
        tracking: {
          meta: buildMetaTrackingPayload(leadEventId),
        },
      };
      ...
      debugLog('submit:start', {
        ...
        selectedProducts: data.queConstruir.length,
      });

      const res = await fetch(`${API_URL}/api/orgs/afluence/bus/ai-factory-creators/ingest`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
      const responsePayload = (await res.json().catch(() => null)) as
        | { lead?: { id?: string }; entries?: Array<{ id?: string }>; message?: string; details?: unknown }
        | null;
      debugLog('submit:response', { status: res.status, ok: res.ok });
      if (!res.ok) {
        const message =
          typeof responsePayload?.message === 'string'
            ? responsePayload.message
            : getValidationMessage(responsePayload?.details) ?? `Request failed (${res.status})`;
      ...
      trackEvent(META_EVENTS.CompleteRegistration, {
        content_name: 'ai-factory-creators-v1',
        form_type: 'full',
      }, { eventId: leadEventId });
      const leadId = responsePayload?.lead?.id;
      const entryId = responsePayload?.entries?.[0]?.id;
      onCalendarUnlocked?.({
        name: data.nombre.trim(),
        email: data.email.trim(),
        meetingUpdate: leadId ? { leadId, entryId } : undefined,
      });
      debugLog('submit:unlocked', { hasCalendlyPrefill: true });
      setSubmitted(true);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'No pudimos enviar tu solicitud';
      debugLog('submit:error', { message });
      ...
      return (
        <div className="h-full flex items-center">
          ...
          >
            ...
            <h3 className="text-[1.25rem] text-brand-text mb-3" style={{ fontWeight: 700 }}>
              Reservamos reuniones solo para proyectos confirmados por WhatsApp
            </h3>
            <p className="text-[0.875rem] text-brand-text-secondary mb-6" style={{ lineHeight: 1.7 }}>
              Entendemos. Si cambias de opinion, puedes unirte a nuestra lista de espera y te
              contactaremos cuando haya disponibilidad.
            </p>
            <button
      ...
      return (
        <div className="h-full flex items-center">
          ...
          >
            ...
            <h3 className="text-[1.25rem] text-brand-text mb-2" style={{ fontWeight: 700 }}>
              Listo, {data.nombre}. Ahora puedes agendar.
            </h3>
            <p className="text-[0.875rem] text-brand-text-secondary mb-1" style={{ lineHeight: 1.7 }}>
              Selecciona una fecha y horario en el calendario para tu sesion estrategica.
            </p>
            <p className="text-[0.75rem] text-brand-muted" style={{ lineHeight: 1.6 }}>
              Recibiras confirmacion por WhatsApp y email.
            </p>
          </motion.div>
        </div>
      );
    ...
    <div className="rounded-2xl border border-brand-border bg-brand-surface/60 backdrop-blur-2xl shadow-[0_8px_60px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col h-full">
      <div className="px-5 pt-5 pb-0 md:px-7 md:pt-7">
          ...
          {stepsMeta.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-1 flex-1">
                ...
                {i < TOTAL_STEPS - 1 && (
                  <div
                    className={`flex-1 h-px mx-1 ${isDone ? 'bg-brand-success/30' : 'bg-brand-border'}`}
                  />
                )}
              </div>
            );
        ...
        <div className="mb-1">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[0.75rem] text-brand-muted" style={{ fontWeight: 500 }}>
              Paso {step} de {TOTAL_STEPS}
            </span>
            <span className="text-[0.6875rem] text-brand-muted/60" style={{ fontWeight: 400 }}>
          ...
          >
            {step === 1 && (
              <div className="flex flex-col gap-4">
                <h3 className="text-[1.0625rem] text-brand-text mb-1" style={{ fontWeight: 600 }}>
                  Cuentanos sobre ti
                </h3>
                ...
                <FormField label="Nicho" required>
                  ...
                  >
                    <option value="" style={{ background: '#1C1C26', color: '#71717A' }}>
                      Seleccionar nicho
                    </option>
                    {nichos.map((n) => (
                      <option key={n} value={n} style={{ background: '#1C1C26', color: '#F5F5F7' }}>
                        {n}
                      </option>
                    ))}
                  </select>
            ...
            {step === 2 && (
              <div className="flex flex-col gap-5">
                ...
                <FormField label="Facturacion mensual aproximada" required>
            ...
            {step === 3 && (
              <div className="flex flex-col gap-5">
                ...
                <p className="text-[0.8125rem] text-brand-muted -mt-2" style={{ lineHeight: 1.6 }}>
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded flex-shrink-0 flex items-center justify-center border transition-all ${
                            isChecked ? 'bg-brand-accent border-brand-accent' : 'border-brand-border'
                          }`}
                        >
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="flex items-start gap-3 p-4 rounded-xl bg-brand-accent/6 border border-brand-accent/15">
                  ...
                  <p className="text-[0.8125rem] text-brand-text-secondary" style={{ lineHeight: 1.7 }}>
                    ...
                    <span className="text-brand-accent" style={{ fontWeight: 600 }}>
                      USD $7,000
                    </span>
                    .
                  </p>
                </div>
                <FormField label="¿Te gustaria invertir en ese rango para tu proyecto?" required>
                  <div className="flex flex-col gap-2">
                    {inversiones.map((inv) => (
                      <RadioButton
                        key={inv.value}
                        ...
                        variant={inv.value === 'lower' && data.inversion === 'lower' ? 'warning' : undefined}
                      />
                    ))}
                  </div>
                </FormField>
              ...
              <div className="flex flex-col gap-5">
                {calendarState === 'rejected' ? (
                  <div className="text-center py-4">
                    ...
                    <h3 className="text-[1.125rem] text-brand-text mb-3" style={{ fontWeight: 700 }}>
                      Nuestros desarrollos comienzan desde USD $7,000
                    </h3>
                    <p className="text-[0.8125rem] text-brand-text-secondary mb-6" style={{ lineHeight: 1.7 }}>
                      Entendemos que puede no ser el momento. Te invitamos a unirte a nuestra lista de espera para cuando estes listo.
                    </p>
                    <button
                  ...
                  <>
                    ...
                    <FormField label="¿Cuando te gustaria lanzar?" required>
                      <div className="grid grid-cols-2 gap-2">
                        {timings.map((t) => (
                          <RadioButton
                            key={t}
                    ...
                    <div className="flex items-start gap-3 p-4 rounded-xl bg-brand-accent/6 border border-brand-accent/15">
                      ...
                      <p className="text-[0.8125rem] text-brand-text-secondary" style={{ lineHeight: 1.7 }}>
                        Al agendar la reunion, debes <span className="text-brand-text" style={{ fontWeight: 500 }}>confirmarla por WhatsApp</span> para que podamos revisar tu proyecto.
                      </p>
                    </div>
                    <FormField label="Confirmacion" required>
                      <div className="flex flex-col gap-2">
                        {whatsappOptions.map((opt) => (
                          <RadioButton
                            ...
                            variant={opt.value === 'no' && data.confirmaWhatsapp === 'no' ? 'warning' : undefined}
                          />
                        ))}
                      </div>
                    </FormField>
      ...
      {!(step === 5 && calendarState === 'rejected') && !submitted && (
        <div className="px-5 pb-5 pt-0 md:px-7 md:pb-7 border-t border-brand-border">
          ...
          {submitError && (
            <p className="text-[0.75rem] text-amber-400 mt-3" role="alert">
              {submitError}
            </p>
          )}
        </div>
...
function getValidationMessage(details: unknown): string | null {
  ...
  const fieldErrors = Object.values(details as Record<string, unknown>);
...
function RadioButton({
  ...
  return (
    ...
    >
      ...
      >
        {selected && (
          <div className={`w-2 h-2 rounded-full ${isWarning ? 'bg-amber-500' : 'bg-brand-accent'}`} />
        )}
      </div>
